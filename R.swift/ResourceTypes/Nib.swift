...
private class NibParserDelegate: NSObject, NSXMLParserDelegate {
    ...
    @objc func parser(parser: NSXMLParser, parseErrorOccurred parseError: NSError) {
        var message = parseError.localizedDescription
        //try extract the xml parser error message.
        if let errorMessage = parseError.userInfo["NSXMLParserErrorMessage"] as? String {
            message = errorMessage
        }
        warn("Error parsing \(filename ?? "") , code: \(parseError.code), Description: \(message), Line: \(parser.lineNumber), Column: \(parser.columnNumber) ")
    }
