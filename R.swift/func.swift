...
func inputDirectories(processInfo: NSProcessInfo) -> [NSURL] {
    return processInfo.arguments.skip(1).map {
...
func storyboardStructForStoryboard(storyboard: Storyboard) -> Struct {
  ...
  let initialViewControllerVar = catOptionals([storyboard.initialViewController.map { iv in
    Var(isStatic: true, name: "initialViewController", type: iv.type.asOptional(), getter: "return instance.instantiateInitialViewController()" + (iv.type.asNonOptional() == Type._UIViewController ? "" : "as? \(iv.type.asNonOptional())") + "")
  }])
