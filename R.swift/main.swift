...
do {
  ...
  let url = callInformation.outputPathURL.URLByAppendingPathComponent(targetType.fileName)

  // Write file if we have changes
  let currentFileContents = try? String(contentsOfURL: url, encoding: NSUTF8StringEncoding)
  if currentFileContents != fileContents  {
    do {
      try fileContents.writeToURL(url, atomically: true, encoding: NSUTF8StringEncoding)
